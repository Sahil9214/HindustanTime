...
// const api = `https://youtube.googleapis.com/youtube/v3/search?q=${query}&key=${API_KEY}&part=snippet&maxResults=20`;



const api = `https://www.googleapis.com/youtube/v3/videos?part=snippet&chart=mostPopular&key=${API_KEY}&maxResults=6&regionCode=IN` 
  
async function news_vedio(){
...
const append4 = (data) => {
    ...
    const container = document.getElementById("news_vedio");
    ...
    data.forEach(({snippet,id})=>{
        ...
        const videoId = document.createElement("iframe");
        videoId.src =  `https://www.youtube.com/embed/${id.videoId}`;
        ...
        video_info.append(name);
    
        div.append(videoId,video_info);
